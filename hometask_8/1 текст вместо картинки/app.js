 ...
 */
function handleClick(clickBtnEvent) {
    // 2. из объекта события получите ссылку на .product и
    // сохраните в переменную:
    const cardNode = clickBtnEvent.target.parentNode;
...
function showMoreText(card) {
    ...
    const informationText = "Lorem ipsum dolor sit amet, consectetur adipisicing elit, consectetur adipisicing elit, sed do eiusmod";
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    let productName = card.productName;
    productName.insertAdjacentHTML('afterend', `<div class="desc">${informationText}</div>`);
